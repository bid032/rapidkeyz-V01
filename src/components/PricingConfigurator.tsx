      ...
      <AnimatePresence>
        {open && (
          <motion.ul
            ...
            className="absolute z-30 mt-2 w-full max-h-72 overflow-auto rounded-2xl border border-white/15 shadow-2xl p-1.5 backdrop-blur-2xl backdrop-saturate-150"
            style={{ backgroundColor: "hsl(var(--card) / 0.55)" }}
          >
            {variants.map((v) => {
              const isSel = v === value;
              return (
