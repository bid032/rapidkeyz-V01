  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <header className="relative overflow-hidden pt-10 pb-16 sm:pt-16 sm:pb-24">
        ...
        {/* Broken-grid canvas: 12 cols, elements overlap and break out */}
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6">
          <div className="grid grid-cols-12 gap-4 sm:gap-6 items-center min-h-[70vh]">

            {/* LEFT , Number marker + kicker */}
            <div className="col-span-12 md:col-span-2 md:pt-8 order-2 md:order-1">
              <div className="flex md:flex-col items-start gap-3">
                <div
                  data-gsap="split-chars"
                  className="font-display font-bold text-6xl sm:text-8xl leading-none text-brand text-glow"
                >
                  01
                </div>
                <div className="hidden md:block h-24 w-px bg-gradient-to-b from-brand via-brand/40 to-transparent" />
                <div className="font-mono text-[10px] sm:text-xs uppercase tracking-[0.3em] text-muted-foreground writing-mode-vertical rotate-180 hidden md:block" style={{ writingMode: "vertical-rl" }}>
                  {hero.badge}
                </div>
                <span className="md:hidden inline-flex items-center gap-2 px-3 py-1 rounded-full neon-border text-brand text-[11px] font-mono uppercase tracking-widest">
            ...
            {/* CENTER , Big headline + description + CTAs */}
            <div className="col-span-12 md:col-span-8 order-1 md:order-2 relative">
              {/* Broken-out floating tag */}
              <div className="hidden md:flex absolute -top-6 -right-4 items-center gap-2 px-3 py-1.5 rounded-full neon-border bg-background/60 backdrop-blur text-brand text-[11px] font-mono uppercase tracking-widest rotate-3">
                <span className="size-1.5 rounded-full bg-brand animate-pulse" />
              ...
              <h1
                ...
                className="font-display font-bold text-[clamp(2.25rem,7vw,5.5rem)] leading-[1.05] tracking-tight text-foreground"
              >
                {hero.title1}
              </h1>
              ...
              <p
                data-gsap="split-words"
                className="mt-4 sm:mt-6 font-display text-[clamp(1.25rem,3vw,2.5rem)] font-semibold leading-tight bg-gradient-to-r from-brand via-accent to-brand bg-clip-text text-transparent"
              >
                {hero.title2}
              </p>
              ...
              <p
                data-gsap="reveal"
                className="mt-6 sm:mt-8 max-w-xl text-muted-foreground text-base sm:text-lg leading-relaxed"
              >
                {hero.subtitle}
              </p>
              ...
              {/* Stat strip broken out below */}
              <div data-gsap="reveal-stagger" className="mt-10 sm:mt-14 flex flex-wrap gap-6 sm:gap-10">
                {[
                  { n: "24/7", l: "دعم فوري" },
                  { n: "+15k", l: "عملاء" },
                  { n: "4.9★", l: "تقييم" },
                  ...
                  <div key={s.l} className="flex items-baseline gap-2">
                    <span className="font-display font-bold text-2xl sm:text-3xl text-brand text-glow">{s.n}</span>
                    <span className="text-xs sm:text-sm text-muted-foreground">{s.l}</span>
                  </div>
                ))}
              </div>
            </div>

            {/* RIGHT — Broken-out floating cards (desktop) */}
            <div className="col-span-12 md:col-span-2 order-3 relative hidden md:block">
              <div
                data-gsap="tilt"
            ...
            {/* Mobile — same floating cards in a horizontal row */}
            <div data-gsap="card-pop" className="col-span-12 order-4 md:hidden flex gap-3 mt-4 -mx-4 px-4 overflow-x-auto no-scrollbar snap-x snap-mandatory">
